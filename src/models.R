...
### Functions for power grid load forecasting

# Create lagged version of megawatt data
...
# Join load data with temperature data, filter to zone
...
# Standardize features for megawatt forecasting
...
# Preprocess data for MW forecasting with GAM
...
# Function to train a GAM to predict MW on a given zone
...
# Function to predict MW at new time points
...
## Neural net peak hour functions

# Standardize data for prediction with neural nets, aggregate data to day level
...
# Preprocess data for peak hour forecasting
...
# Function to train neural net to predict peak hour of day
...
# Predict peak hours for new time points
...
## Functions for neural net forecasting of peak days

# Standardize feature variables, aggregate to day level
...
# Preprocess data for neural net predictions
...
# Function to train a neural net to forecast peak days
train_pd = function(load_data, temp_data, zone, mw_preds=NULL) {
  ...
  nn = nnet(factor(is_peak_day_in_week) ~ dayofyear_ept + year_ept + weekday_ept
            ...
            data=prepped_data, weights=peak_in_day, size=10, maxit=1000)
  
  return(list(nn, zone, summary_vars))
}

# Function to predict peak days for new observations
