...
predict_mw = function(gm, oldload, oldtemp, newload, newtemp, zone, summary_vars) {
  old_data = load_join_temp(oldload, oldtemp, zone) %>% 
    select(date_ept, hour24_ept, old_mw = mw)
  old_data = old_data %>% mutate(date_ept = date_ept + years(1))
  pred_data = load_join_temp(newload, newtemp, zone)
  pred_data = left_join(pred_data, old_data, by=join_by(date_ept, hour24_ept)) %>% 
    mutate(mw = old_mw)
  ...
  pred_data = pred_data %>% mutate(mw = preds[[1]])
  pred_standard = standardize_features(pred_data, summaries=summary_vars)
  preds = predict(gm, newdata=pred_standard)
  return(data.frame(predicted_mw = preds, pred_data))
}
...
ph_prepdata = function(load_data, temp_data, zone, preds=NULL) {
  ...
  group_by(date_ept) %>% 
    ...
    mutate(peakhour = ceiling((which.max(mw)-1)/3)*3-1) %>% ungroup() 
...
pd_standardize = function(prepped, summary_vars) {
  prepped = prepped %>% 
    ...
    summarize(peak_in_day = max(mw),
              ...
              tempF11 = max(tempF11),
              tempF12 = max(tempF12),
              tempF0_sq = max(tempF0_sq),
              tempF1_sq = max(tempF1_sq),
              tempF2_sq = max(tempF2_sq),
              tempF3_sq = max(tempF3_sq),
              tempF4_sq = max(tempF4_sq),
              tempF5_sq = max(tempF5_sq),
              tempF6_sq = max(tempF6_sq),
              tempF7_sq = max(tempF7_sq),
              tempF8_sq = max(tempF8_sq),
              tempF9_sq = max(tempF9_sq),
              tempF10_sq = max(tempF10_sq),
              tempF11_sq = max(tempF11_sq),
              tempF12_sq = max(tempF12_sq)) %>% ungroup()
...
predict_pd = function(nn, newload, newtemp, zone, summary_vars, mw_preds) {
  ...
  pred_standard = pd_standardize(pred_data, summaries)
